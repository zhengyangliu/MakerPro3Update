...
(function(ext) {
    ...
    ext.initBrushless = function(port){
    	if(typeof port=="string"){
    		port = ports[port];
    	}
    	runPackage(68,01,port);
    }
    ext.runBrushless = function(port,power){
    	if(typeof port=="string"){
    		port = ports[port];
    	}
    	runPackage(68,02,port,Math.abs(power));
    }
	...
	ext.runNewMotor = function(port,speed){
		if(typeof port=="string"){
			port = ports[port];
		}
        runPackage(65,port,short2array(speed));
	}
    ...
    ext.runServo = function(slot,angle) {
    	var servoSlots = {
    		"M1 ":1,
    		"M2 ":2,
    		"M3 ":3,
		...
		if(typeof slot=="string"){
			slot = servoSlots[slot];
		}
		if(angle > 180){
			angle = 180;
		}
        runPackage(11,slot,angle);
    };
	...
	ext.runI2CMotorPower = function(addr,slot,power){
		...
		runPackage(69,03,addr+9,slot,short2array(power));
	}
	ext.runI2CMotorSpeed = function(addr,slot,speed){
		if(typeof addr=="string"){
			addr = ports[addr];
		}
		if(typeof slot=="string"){
		...
		runPackage(69,02,addr+9,slot,short2array(speed));
	};
	ext.runI2CMotorMoveTo = function(addr,slot,degrees,speed){
		if(typeof addr=="string"){
			addr = ports[addr];
		}
		if(typeof slot=="string"){
		...
		runPackage(69,01,addr+9,slot,int2array(degrees),short2array(speed));
	};
	...
	ext.getColorValue = function(nextID,port,colorType){
    	var deviceId = 67;
		var map = {"R":0,
		"G":1,
		"B":2,
		...
		{
			colorType = map[colorType]||0;
		}
		
		if(typeof port=="string"){
			port = ports[port];
		}
		getPackage(nextID,deviceId,port,01,colorType);
    };
	ext.getColorType = function(nextID,port,colorType){
    	var deviceId = 67;
		var map = {
		"white":0,
		...
		getPackage(nextID,deviceId,port,02,colorType);
    };
	...
	ext.getI2CMotorSpeed = function(nextID,addr,slot){
		var deviceId = 69;
		if(typeof addr=="string"){
			addr = ports[addr];
		}
    	...
    	getPackage(nextID,deviceId,0x11,9+addr,slot);
    };
	ext.getI2CMotorPosition = function(nextID,addr,slot){
		var deviceId = 69;
		if(typeof addr=="string"){
			addr = ports[addr];
		}
    	...
    	getPackage(nextID,deviceId,0x10,9+addr,slot);
    };
    ext.isAutoMode = function(nextID){
		var deviceId = 60;
    	getPackage(nextID,deviceId,0x75);
    }
